...
/**
 * `?anh=<địa chỉ>` — đo bằng ẢNH JPEG THẬT qua HTTP thay vì SVG nhúng sẵn.
 * SVG nhúng hiện ra tức thì nên giấu mất đúng thứ khách kêu: ô trắng lúc lướt
 * ngược lên, vì ảnh phải tải lại. Mỗi ô lấy một địa chỉ riêng (?i=) để trình
 * duyệt coi là ảnh khác nhau, dù trên đĩa chỉ có 20 file.
 */
function anhThat(base: string, i: number): string {
  return `${base}/${i % 20}.jpg?i=${i}`;
}
...
export default function LuoiAnhDemo() {
  const srcOf = useRef(new Map<string, string>());
  const masonry = useMasonry(2, 4, (id) => srcOf.current.get(id));
  ...
  const photos = useMemo(
    () =>
      Array.from({ length: total }, (_, i) => {
        const p = fakePhoto(i);
        return anhBase ? { ...p, src: anhThat(anhBase, i) } : p;
      }),
    [total, anhBase]
  );
  const ids = useMemo(() => {
  ...
  useEffect(() => {
    const tick = () => {
      setHeld(document.querySelectorAll("#luoi-anh-demo img").length);
